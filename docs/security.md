...
Structured JSON logs generated by `backend/src/middleware/logger.js` automatically redact sensitive fields before outputting to stdout or log aggregators:

```json
{
  "timestamp": "2026-09-19T13:20:00.000Z",
...
- ID numbers are masked to only show the last 4 characters.
- Base64 image strings and raw file buffers are omitted from log events.
